...
Service_mtproxy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/cannotbenormal/doubi/master/service/mtproxy_centos" -O /etc/init.d/mtproxy; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/cannotbenormal/doubi/master/service/mtproxy_debian" -O /etc/init.d/mtproxy; then
			echo -e "${Error} MTProxy服务 管理脚本下载失败 !"
			rm -rf "${file}"
			exit 1
		fi
...
Set_mtproxy(){
 ...
 ${Green_font_prefix} 2.${Font_color_suffix}  Modify proxy secret	
 ${Green_font_prefix} 3.${Font_color_suffix}  Modify proxy TAG
 ${Green_font_prefix} 4.${Font_color_suffix}  Modify NAT configuration
 ${Green_font_prefix} 5.${Font_color_suffix}  Modify whole config
————————————————
 ${Green_font_prefix} 6.${Font_color_suffix}  Update Telegram IP base
 ${Green_font_prefix} 7.${Font_color_suffix}  Update Telegram Key
————————————————
 ${Green_font_prefix} 8.${Font_color_suffix}  Scheduled Telegram IP base update
 ${Green_font_prefix} 9.${Font_color_suffix}  Proxy operation status monitor
 ${Green_font_prefix}10.${Font_color_suffix}  Main IP change monitor" && echo
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/cannotbenormal/doubi/master/mtproxy.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/mtproxy" ]]; then
		rm -rf /etc/init.d/mtproxy
		Service_mtproxy
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/cannotbenormal/doubi/master/mtproxy.sh" && chmod +x mtproxy.sh
...
————————————
 ${Green_font_prefix} 7.${Font_color_suffix} Proxy configuration
 ${Green_font_prefix} 8.${Font_color_suffix} View proxy info
 ${Green_font_prefix} 9.${Font_color_suffix} View proxy log
 ${Green_font_prefix}10.${Font_color_suffix} View linked IPs
————————————" && echo
