...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/cannotbenormal/ssrrmu/master/bbr.sh; then
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/cannotbenormal/ssrrmu/master/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/cannotbenormal/ssrrmu/master/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
Install_LotServer(){
	...
	wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/cannotbenormal/serverSpeeser_Install/master/appex.sh"
...
Uninstall_LotServer(){
	...
	[[ -z ${unyn} ]] && echo && echo "已取消..." && exit 1
	if [[ ${unyn} == [Yy] ]]; then
		wget --no-check-certificate -qO /tmp/appex.sh "https://raw.githubusercontent.com/cannotbenormal/serverSpeeser_Install/master/appex.sh" && bash /tmp/appex.sh 'uninstall'
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/cannotbenormal/ssrrmu/master/ssrrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/cannotbenormal/ssrrmu/master/ssrrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		if [[ ${yn} == [Yy] ]]; then
			...
			if [[ $sh_new_type == "github" ]]; then
				wget -N --no-check-certificate https://raw.githubusercontent.com/cannotbenormal/ssrrmu/master/ssrrmu.sh && chmod +x ssrrmu.sh
			fi
